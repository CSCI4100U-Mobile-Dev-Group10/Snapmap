...
class _FriendRequestSenderState extends State<FriendRequestSender> {
  ...
  checkifUserExists(String us) async {
    var result = await UserService.getInstance().getOtherUser(us);
    // verify user exists
            ...
            Positioned(
              ...
              child: TextFormField(
                ...
                validator: (value) {
                  ...
                  } else if (_user.sentFriendRequests
                      .contains(value.toString())) {
                    return 'You have already sent a request to ${value.toString()}';
                  } else if (_user.receivedFriendRequests
                      .contains(value.toString())) {
              ...
              child: ElevatedButton(
                onPressed: () async {
                  if (_formKey.currentState!.validate()) {
                    ...
                    _controller.clear();
                    var value = await UserService.getInstance()
                        .getOtherUser(usernameToSend);
                    if (value != null) {
                      await UserService.getInstance()
                          .requestFriend(usernameToSend);
                    } else {
